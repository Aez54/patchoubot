      ...
      if (!channel) {
        ...
        if (channel.permissionsFor(client.user).has('SEND_MESSAGES')) {
          ...
          if (typeof publicmsg === "object") {
            ...
            const emoji = client.emojis.cache.get("916320697553412169")
            const emojis = client.emojis.cache.get("916313566720720947")

            msg = publicmsg.replace(`@MEMBER`, `${member.user}`);
            msg = msg.replace(`@GUILDNAME`, `${member.guild.name}`);
            msg = msg.replace(`@GUILDMEMBER`, `<@&854026908287369241>`);
            msg = msg.replace(`@EMOJI`, `${emoji} `);
            msg = msg.replace(`@EMOJIS`, `${emojis}`);
            msg = msg.replace(`@REGLEMENT`, `<#895342853885161513>`);
            msg = msg.replace(`@ROLE`, `<#854026908487385107>`);
            msg = msg.replace(`@PRESENTATION`, `<#854026908487385106>`);
